...
class EventWarping(torch.nn.Module):
    ...
    def event_flow_association(self, flow_list, event_list, pol_mask, event_mask):
        ...
        event_idx = event_list[:, :, 1:3].clone() # pixel coordinate of each event, torch.Size([bs, 1000, 2])
        event_idx[:, :, 0] *= self.res[1]  # torch.view is row-major, multiple the number of row with resolution to get a 1-d index
        event_idx = torch.sum(event_idx, dim=2) # add 128*u+v to get a 1-d index [bs, 1000] of where the event is
        ...
        for i, flow in enumerate(flow_list):
            flow = flow.view(flow.shape[0], 2, -1) # torch.Size([bs, 2, 128*128])
            # gather: event_flowy[0, i] = flow[:, 1, :][0, event_idx.long()[0, i]]  可理解为遍历index的每一个元素，这个元素的值作为在input中采样的位置index。采样得到的out tensor和index形状相同。
            event_flowy = torch.gather(flow[:, 1, :], 1, event_idx.long())  # vertical component # self.long() is equivalent to self.to(torch.int64)
            event_flowx = torch.gather(flow[:, 0, :], 1, event_idx.long())  # horizontal component 
    ...
    def overwrite_intermediate_flow(self, flow_list):
        ...
        # flow vector per input event
        event_idx = self._event_list[:, :, 1:3].clone()
        event_idx[:, :, 0] *= self.res[1]  # torch.view is row-major
        event_idx = torch.sum(event_idx, dim=2)
        ...
        for flow in flow_list:
            ...
            event_flowy = torch.gather(flow[:, 1, :], 1, event_idx.long())  # vertical component
            event_flowx = torch.gather(flow[:, 0, :], 1, event_idx.long())  # horizontal component
            event_flowy = event_flowy.view(event_flowy.shape[0], event_flowy.shape[1], 1)
            event_flowx = event_flowx.view(event_flowx.shape[0], event_flowx.shape[1], 1)
            event_flow = torch.cat([event_flowy, event_flowx], dim=2)
...
class BaseValidationLoss(torch.nn.Module):
    ...
    def event_flow_association(self, flow_list, inputs):
        ...
        # flow vector per input event
        event_idx = event_list[:, :, 1:3].clone()
        event_idx[:, :, 0] *= self.res[1]  # torch.view is row-major
        event_idx = torch.sum(event_idx, dim=2)
