...
def load_model(prev_runid, model, device):
    ...
    model_dir = os.getcwd() + run.info.artifact_uri + "/model_best/data/model.pth" # model_best model
    if model_dir[:7] == "file://":
        model_dir = model_dir[7:]
    if os.path.isfile(model_dir):
        print("Model restored from ", model_dir)

        model_loaded = torch.load(model_dir, map_location=torch.device('cpu'))
...
def save_model_latest(model):
    mlflow.pytorch.log_model(model, "model")

def save_model_tail(model, epoch):
    mlflow.pytorch.log_model(model, "model_"+str(epoch))
