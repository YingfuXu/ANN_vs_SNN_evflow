...
class ConvGRU(nn.Module):
    """
    Convolutional GRU cell. https://en.wikipedia.org/wiki/Gated_recurrent_unit Fully gated unit
...
class ConvRecurrentSoftsign(nn.Module):
    """
    Convolutional recurrent cell (for direct comparison with spiking nets).
    """
    ...
    def __init__(self, input_size, hidden_size, kernel_size, activation=None):
        ...
        self.softsign = nn.Softsign() # to replace tanh for better SENeCA compatibility

        print("Softsign for RNN activation without thresholding.")
    ...
    def forward(self, input_, prev_state):
        ...
        ff = self.ff(input_)
        rec = self.rec(prev_state)
        state = self.softsign(ff + rec)
        out = self.out(state)
        out = torch.relu(out)

        return out, state
    
class ConvRecurrentSoftsignThreshold(nn.Module): # 
    """
    Convolutional recurrent cell (for direct comparison with spiking nets).
    """
    ...
    def __init__(self, input_size, hidden_size, kernel_size, activation=None, NoConvState=False):
        ...
        if NoConvState:
            ...
            self.scale_hidden_state = nn.Parameter(torch.ones(1) * 1e-6) # initialize with a random selected value
        else:
            self.rec = nn.Conv2d(input_size, hidden_size, kernel_size, padding=padding)
        self.out = nn.Conv2d(input_size, hidden_size, kernel_size, padding=padding)
        ...
        print("There are thresholds for the activation function of recurrent neuron states!")
        ...
        if NoConvState:
            print("No rec=self.rec(prev_state)! rec=prev_state")
    ...
    def forward(self, input_, prev_state):
        ...
        # if self.train_thre_acti_rec:
        activation_map = binarize(torch.abs(state)-torch.abs(self.thre_acti_rec)) # silencing neurons with activation close to zero, abs below |thre_acti_rec|
...
class ConvRecurrentFATReLU(nn.Module): # 
    """
    Convolutional recurrent cell (for direct comparison with spiking nets).
    """
    ...
    def __init__(self, input_size, hidden_size, kernel_size, activation=None, NoConvState=False):
        ...
        self.train_thre_acti_rec = True # if True, thre_acti_rec is trainable
        print("FATReLU for the activation function of recurrent neuron states!")
...
class ConvRecurrentGradedLIF(nn.Module): # 
    ...
    def forward(self, input_, prev_state):
        ...
        ff = self.ff(input_)
        # generate zeros prev_state, if None is provided
        if prev_state is None:
        ...
        # print(torch.stack([out_LIF, state]).size()) # torch.Size([2, 8, 32, 128, 128])

        return out, torch.stack([out_LIF, state])
    

class ConvRecurrentS(nn.Module): # 
    """
    Convolutional recurrent cell (for direct comparison with spiking nets).
    """
