...
class FireNet(BaseModel): # original
    """
...
class FireNet_Sparsify(BaseModel):
    ...
    head_neuron = ConvLayer_
    ff_neuron = ConvLayer_
    rec_neuron = ConvRecurrentFATReLU_2Conv
    residual = False
    num_recurrent_units = 7
    kwargs = [{}] * num_recurrent_units
    w_scale_pred = None

    def __init__(self, firenet_kwargs):
        super().__init__()
        self.num_bins = firenet_kwargs["num_bins"]
        base_num_channels = firenet_kwargs["base_num_channels"]
        kernel_size = firenet_kwargs["kernel_size"]
        self.encoding = firenet_kwargs["encoding"]
        self.norm_input = False if "norm_input" not in firenet_kwargs.keys() else firenet_kwargs["norm_input"]
        self.mask = firenet_kwargs["mask_output"]
        ff_act, rec_act = firenet_kwargs["activations"]
        if type(firenet_kwargs["spiking_neuron"]) is dict:
            for kwargs in self.kwargs:
                kwargs.update(firenet_kwargs["spiking_neuron"])
        ...
        self.pred = ConvLayer(
            base_num_channels, out_channels=2, kernel_size=1, activation=None, w_scale=self.w_scale_pred # "tanh"
        )
        self.softsign = torch.nn.Softsign()
        if self.train_FATReLU:
            self.thre_x1 = torch.nn.Parameter(torch.ones(base_num_channels, 1, 1) * 1e-6) # different threshold for each channel
            self.thre_x2 = torch.nn.Parameter(torch.ones(base_num_channels, 1, 1) * 1e-6)
            self.thre_x3 = torch.nn.Parameter(torch.ones(base_num_channels, 1, 1) * 1e-6)
            self.thre_x4 = torch.nn.Parameter(torch.ones(base_num_channels, 1, 1) * 1e-6)
            self.thre_x5 = torch.nn.Parameter(torch.ones(base_num_channels, 1, 1) * 1e-6)
            self.thre_x6 = torch.nn.Parameter(torch.ones(base_num_channels, 1, 1) * 1e-6)
            self.thre_x7 = torch.nn.Parameter(torch.ones(base_num_channels, 1, 1) * 1e-6)
        else:
    ...
    def forward(self, event_voxel, event_cnt, log=False):
        ...
        if not self.is_SNN: # when ANN
            state_1 = x1.clone()
            if self.train_FATReLU: # ANN ONLY
                activation_map = binarize(x1-torch.abs(self.thre_x1))
            else:
                activation_map = (x1-self.FATReLU_threshold[1]).gt(0).float()
            x1 = x1 * activation_map
        ...
        if not self.is_SNN: # when ANN
            state_2 = x2.clone()
            if self.train_FATReLU:
                activation_map = binarize(x2-torch.abs(self.thre_x2))
            else:
        ...
        if not self.is_SNN: # when ANN
            state_3 = x3.clone()
            if self.train_FATReLU:
                activation_map = binarize(x3-torch.abs(self.thre_x3))
            else:
                activation_map = (x3-self.FATReLU_threshold[3]).gt(0).float()
            x3 = x3 * activation_map

        x4, self._states[3] = self.R1b(x3, self._states[3], residual=x2 if self.residual else 0)
        if not self.is_SNN: # when ANN
            state_4 = x4.clone()
            if self.train_FATReLU:
                activation_map = binarize(x4-torch.abs(self.thre_x4))
            else:
                activation_map = (x4-self.FATReLU_threshold[4]).gt(0).float()
            x4 = x4 * activation_map

        x5, self._states[4] = self.G2(x4, self._states[4])
        if not self.is_SNN: # when ANN
            state_5 = x5.clone()
            if self.train_FATReLU:
                activation_map = binarize(x5-torch.abs(self.thre_x5))
            else:
        ...
        if not self.is_SNN: # when ANN
            state_6 = x6.clone()
            if self.train_FATReLU:
                activation_map = binarize(x6-torch.abs(self.thre_x6))
            else:
                activation_map = (x6-self.FATReLU_threshold[6]).gt(0).float()
            x6 = x6 * activation_map

        x7, self._states[6] = self.R2b(x6, self._states[6], residual=x5 if self.residual else 0)
        if not self.is_SNN: # when ANN
            state_7 = x7.clone()
            if self.train_FATReLU:
                activation_map = binarize(x7-torch.abs(self.thre_x7))
            else:
                activation_map = (x7-self.FATReLU_threshold[7]).gt(0).float()
            x7 = x7 * activation_map

        flow = self.softsign(self.pred(x7))

        # log activity
        if log:
            activity = {}
            acti_after_thresholding = {}
            ...
            if self.training:
                if self.is_SNN:
                    ...
                    for n, l in zip(name, [x, (self._states[0][0]), (self._states[1][0]), (self._states[2][0]), (self._states[3][0]), (self._states[4][0]), (self._states[5][0]), (self._states[6][0]), flow]):
                        activity[n] = l
                    for n, l in zip(name, [x, x1, x2, x3, x4, x5, x6, x7, flow]):
                        acti_after_thresholding[n] = l
                else: # for ANN
                    for n, l in zip(name, [x, state_1, state_2, state_3, state_4, state_5, state_6, state_7, flow]): # for ann neuron state after ReLU before FATReLU
                        activity[n] = l
                    for n, l in zip(name, [x, x1, x2, x3, x4, x5, x6, x7, flow]): # for ann neuron state after FATReLU
                        acti_after_thresholding[n] = l
            else:
                if not self.is_SNN: # ANN
                    for n, l in zip(name, [x, x1, x2, x3, x4, x5, x6, x7, flow]):
                        activity[n] = l
        ...
        if acti_after_thresholding is not None:
