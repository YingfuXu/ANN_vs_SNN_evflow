...
def events_to_image(xs, ys, ps, sensor_size=(180, 240), accumulate=True):
    ...
    img.index_put_((ys, xs), ps, accumulate=accumulate) # https://pytorch.org/docs/stable/generated/torch.Tensor.index_put_.html 

    return img # the elements can be bigger than one, if within this time slot this pixel has more than one event of the same polarity
...
def get_hot_event_mask(event_rate, idx, max_px=100, min_obvs=5, max_rate=0.8):
    ...
    if idx > min_obvs:
        for i in range(max_px):
            ...
            index = (argmax // event_rate.shape[1], argmax % event_rate.shape[1]) # index = (torch.div(argmax, event_rate.shape[1], rounding_mode='floor'), argmax % event_rate.shape[1])
