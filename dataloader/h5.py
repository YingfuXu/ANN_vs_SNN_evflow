...
class H5Loader(BaseDataLoader):
    def __init__(self, config, num_bins, round_encoding=False):
        super().__init__(config, num_bins, round_encoding) # init the base class
        ...
        for root, dirs, files in os.walk(config["data"]["path"]):
            for file in files:
                if file.endswith(".h5"):
                    self.files.append(os.path.join(root, file)) # list of file paths
    ...
    def __getitem__(self, index):
        ...
        while True:
            batch = index % self.config["loader"]["batch_size"] # index of sample in a batch
            ...
            if (self.config["data"]["mode"] == "events" and xs.shape[0] < self.config["data"]["window"]) or (
                ...
                restart = True # for the last samlpe in a sequence. for each sample in a batch, it runs. For example, batch size is 8, it runs 8 times.
            ...
            if restart:
                ...
                # print(self.seq_num)
                # print("restart in __getitem__")

                self.new_seq = True
                self.reset_sequence(batch) # for each sample in a batch, it runs. For example, batch size is 8, seq_num+=8
            ...
            # events to tensors
            event_cnt = self.create_cnt_encoding(xs, ys, ps) # an image of number of event per polarity in each pixel within the time slot
